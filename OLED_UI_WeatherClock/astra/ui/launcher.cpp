...
bool Launcher::open() {
  ...
  if (currentMenu->getType() == "Weather") {
    auto* weather = dynamic_cast<Weather*>(currentMenu);
    weather->resetEnterAnimation();
  }
  if (currentMenu->getType() == "WiFi") {
    ...
    for (unsigned char i = 0; i < key::KEY_NUM; i++) {
      if (HAL::getKeyMap()[i] == key::CLICK) {
        ...
        if (i == 0) { widget_selector = Widget_Selector_NoSelect;close(); }//退出当前项目
          ...
          if (currentMenu->getType() == "WiFi"&&MyWiFi->getSelectedItem()==1) {
            MyWiFi->setWiFiSwitch();
            if (MyWiFi->getWiFiSwitchState()) {
              MyWiFi->popInfoConnect("Connect to WiFi...");
              MyClock->setTime();
            }else if(!MyWiFi->getWiFiSwitchState()) {

              MyWiFi->popInfoDisConnect("Disconnectting...");
            }
            return;
          }
          if (currentMenu->getType() == "Clock") {
              switch (MyClock->getSelectedItem()){
                  ...
                  case Clock::SETTING:
                      ...
                      break;
                  case Clock::STW:
                      MyClock->StopwatchPopup("Stopwatch");
                      break;
              }
