...
#include "menu.h"
#include "SerialMonitor_app.h"  // 引入 SerialMonitor 类的声明
/**
 *     ·  ·     ·   ·
 *  ·   ·  ·  ·   ·
 *  循此苦旅，直抵群星。
...
std::vector<unsigned char> Menu::generateDefaultPic() {
  this->picDefault.resize(120, 0xFF);//120
...
unsigned char Menu::getItemNum() const { return childMenu.size(); }
  ...
  // 安全获取子 Widget 的数量
  unsigned char Menu::getWidgetNum() const {return childWidget.size();}
...
void List::childPosInit(const std::vector<float> &_camera) {
  ...
  for (auto _iter : childMenu) {
    if (_iter->position.xTrg > 200) {
      HAL::printInfo("WARNING: Tile xTrg too large: " + std::to_string(_iter->position.xTrg));
    }
